...
export async function POST({ request }) {
  ...
  try {
    ...
    if (!user) return sendResponse(false, null, null, "Utilisateur non enregistré. Merci de vous inscrire.");
    // on teste la validité du mot de passe saisi
    const passwordCorrect = await bcrypt.compare(password, user?.password);
    // s'il ne l'est pas, on renvoie un message d'erreur
    if (!passwordCorrect) {
      return sendResponse(false, null, null, "Mot de passe incorrect");
    }
